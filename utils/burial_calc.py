...
def torch_generate_raycast_seeds(points, batch_size):
    # Find a point outside the bounding box of the point cloud
    max_abs_coords = torch.max(torch.abs(points), dim=0).values

    # sampled point needs to be sufficiently far from the nearest point so offset by some random large-ish distance.
    outside_point = max_abs_coords + Uniform(25, 100).sample((batch_size, 3)).to(device=max_abs_coords.device, dtype=max_abs_coords.dtype)

    # Generate a random 3D rotation matrix
    rotation = torch.from_numpy(R.random(num=batch_size).as_matrix()).to(device=max_abs_coords.device, dtype=max_abs_coords.dtype) 

    # Apply random rotation to the point
    rotated_point = torch.bmm(rotation, outside_point.unsqueeze(-1)).squeeze(-1)

    return rotated_point


def point_inside_mesh_raycast_pytorch_gpu(
        test_points_tensor: torch.Tensor, pos_tensor: torch.Tensor, 
        triangles_tensor: torch.Tensor, random_outside_point_tensor: torch.Tensor
):
    """
    Determines whether each point in a batch of test points is inside a 3D triangle mesh.

    Parameters:
    - test_points (array): points to test, shape (N, 3).
    - pos (array): vertex positions of the mesh, shape (V, 3).
    - triangles (array): indices that constitute the mesh triangles, shape (T, 3).
    ...
    # Prepare the array of triangles for PyTorch computation
    triangle_vertices = pos_tensor[triangles_tensor].to(test_points_tensor.dtype)
...
def compute_fast_ligand_burial_mask_gpu(ca_coords_: torch.Tensor, lig_coords_: torch.Tensor, alpha: float = 9.0, num_rays: int = 1):
    # Copy the coords to translate non-destructively
    lig_coords = lig_coords_.clone()
    ca_coords = ca_coords_.clone()
    cpu_ca_coords = ca_coords.to(dtype=torch.float32, device='cpu').numpy()
    ...
    # Compute the convex hull of the CA atoms
    triangles = torch.from_numpy(compute_alpha_hull(cpu_ca_coords, alpha)).long()

    # Generate points far away from point cloud from which to draw rays.
    raycast_seeds = torch_generate_raycast_seeds(ca_coords, num_rays)

    lig_mask = point_inside_mesh_raycast_pytorch_gpu(lig_coords, ca_coords, triangles, raycast_seeds[0])
    for idx in range(1, num_rays):
        lig_mask = lig_mask & point_inside_mesh_raycast_pytorch_gpu(lig_coords, ca_coords, triangles, raycast_seeds[idx])

    return lig_mask


def batch_compute_fast_ligand_burial_mask_gpu(
        ca_coords_: torch.Tensor, lig_coords_: torch.Tensor, alpha: float = 9.0, num_rays: int = 1, max_batch_size: int = 1024
):
    # Copy the coords to translate non-destructively
    lig_coords = lig_coords_.clone()
    ca_coords = ca_coords_.clone()
    ...
    # Split the ligand coordinates into manageable chunks for batch processing
    lig_coords_chunks = torch.chunk(lig_coords, max(lig_coords.shape[0] // max_batch_size, 1))

    all_chunk_masks = []
    for chunk in lig_coords_chunks:
        chunk_mask = point_inside_mesh_raycast_pytorch_gpu(chunk, ca_coords, triangles, raycast_seeds[0])
        for idx in range(1, num_rays):
            chunk_mask = chunk_mask & point_inside_mesh_raycast_pytorch_gpu(chunk, ca_coords, triangles, raycast_seeds[idx])
        all_chunk_masks.append(chunk_mask)
    
    return torch.cat(all_chunk_masks)
